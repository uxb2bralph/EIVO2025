...
{
    public class CsvAllowanceUploadManager : CsvUploadManager<ApplicationDbContext, InvoiceAllowance, ItemUpload<InvoiceAllowance>>
    {
        ...
        public CsvAllowanceUploadManager(GenericDbContext<ApplicationDbContext> manager, int sellerID)
            : base(manager)
        {
            _sellerID = sellerID;
        }
        ...
        protected override void doSave()
        {
            ...
            this.EntityList.AddRange(items);
            this.SubmitChanges();
        ...
        protected override bool validate(ItemUpload<InvoiceAllowance> item)
        {
            ...
            item.Entity.InvoiceAllowanceDetails.Add(new InvoiceAllowanceItem
            {
                TaxType = 1,
                No = 0
            });
            ...
            if (invItem != null)
            {
                ...
                else
                {
                    ...
                    //item.Entity.Doc.PushStepQueueOnSubmit(this, Naming.InvoiceStepDefinition.已接收資料待通知, Naming.InvoiceProcessType.G0401);
                }
            }
        ...
        private void checkAmountValue(ItemUpload<InvoiceAllowance> item, InvoiceItem invItem)
        {
            ...
            if (decimal.TryParse(column[(int)FieldIndex.欲折讓單價未稅], out costAmt))
            {
                ...
                {
                    var details = item.Entity.InvoiceAllowanceDetails.First();
                    details.Amount = totalAmt;
                    details.Piece = piece;
                    details.Tax = taxAmt;
                    details.UnitCost = costAmt;
                }
                else
                {
        ...
        private void checkDateValue(ItemUpload<InvoiceAllowance> item)
        {
            ...
            {
                item.Entity.InvoiceAllowanceDetails.First().InvoiceDate = invoiceDate;
            }
            else
            {
        ...
        private InvoiceItem? checkInvoiceNo(ItemUpload<InvoiceAllowance> item)
        {
            ...
            if (item.Columns[(int)FieldIndex.欲折讓發票號碼].Length != 10 || !ValidityAgent.ValidateString(item.Columns[(int)FieldIndex.欲折讓發票號碼], 14))
            {
            ...
            //    _bResult = false;
            //}
            //if (String.IsNullOrEmpty(item.Columns[(int)FieldIndex.原發票email]) && !ValidityAgent.ValidateString(item.Columns[(int)FieldIndex.原發票email], 16))
            //{
            ...
            var details = item.Entity.InvoiceAllowanceDetails.First();
            var product = invItem.Product.First();
            var productItem = product.InvoiceProductItem.First();
            details.TaxType = productItem.TaxType ?? invItem!.InvoiceAmountType!.TaxType;
            details.ProductItemID = productItem.ItemID;
            details.InvoiceNo = invItem.TrackCode + invItem.No;
            if (details.InvoiceDate.HasValue && invItem!.InvoiceDate!.Value.Date != details.InvoiceDate.Value.Date)
            {
                item.Status = String.Join("、", item.Status, "發票日期錯誤");
                _bResult = false;
            }
        ...
        private void checkInputFields(ItemUpload<InvoiceAllowance> item)
        {
            ...
            {
                item.Entity.InvoiceAllowanceDetails.First().OriginalDescription = column[(int)FieldIndex.欲折讓原品名];
            }

            if (String.IsNullOrEmpty(column[(int)FieldIndex.序號]) || !ValidityAgent.ValidateString(column[0], 20))
            {
                item.Status = String.Join("、", item.Status, "序號格式錯誤");
                _bResult = false;
            }

            //if (String.IsNullOrEmpty(column[(int)FieldIndex.客戶ID]) || !ValidityAgent.ValidateString(column[(int)FieldIndex.客戶ID], 20))
            //{
            //    item.Status = String.Join("、", item.Status, "客戶ID格式錯誤");
            //    _bResult = false;
            //}

            if (String.IsNullOrEmpty(column[(int)FieldIndex.折讓單號碼]) || !ValidityAgent.ValidateString(column[(int)FieldIndex.折讓單號碼], 14))
            {
