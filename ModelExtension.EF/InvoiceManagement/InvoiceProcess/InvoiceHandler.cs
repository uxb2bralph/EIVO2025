...
namespace ModelCore.InvoiceManagement.InvoiceProcess
{
    public class InvoiceHandler
    {
        ...
        private GenericDbContext<ApplicationDbContext> models;
        private DbSet<DataProcessQueue> _table;

        public InvoiceHandler(GenericDbContext<ApplicationDbContext> models)
        {
            this.models = models;
        ...
        public static DataProcessQueue? GetReadyItem(InvoiceHandler handler, Naming.InvoiceStepDefinition stepID, Naming.DocumentTypeDefinition docType)
        {
            lock (typeof(InvoiceHandler))
            {
                ...
                DataProcessQueue? item = handler._table!
                    .Where(q => q.StepID == (int)stepID)
                    .Where(q => q.Doc.DocType == (int)docType)
                    .Where(q => q.DispatchDate < DateTime.Now)
                    .Where(q => !q.BookingTime.HasValue || q.BookingTime < available)
                    .FirstOrDefault();
        ...
        private int WriteF0401ToTurnkey(DataProcessQueue item)
        {
            ...
            try
            {
                ...
                if (invoiceItem.Seller!.OrganizationStatus?.DownloadDataNumber == true)
                {
                ...
                if (invoiceItem.Seller!.OrganizationStatus?.DownloadDispatch == true)
                {
                    item.Doc.PushStepQueueOnSubmit(models, Naming.InvoiceStepDefinition.回傳MIG, Naming.InvoiceProcessType.F0401);
                    models.SubmitChanges();
                }
        ...
        private int WriteG0401ToTurnkey(DataProcessQueue item)
        {
            ...
            try
            {
                ...
                if (allowance.InvoiceAllowanceSeller.Seller.OrganizationStatus.DownloadDispatch == true)
                {
                    item.Doc.PushStepQueueOnSubmit(models, Naming.InvoiceStepDefinition.回傳MIG, Naming.InvoiceProcessType.G0401);
                    models.SubmitChanges();
                }
        ...
        private int NotifyIssuedAllowance(DataProcessQueue item)
        {
            ...
            try
            {
                ...
                item.Doc.PushLogOnSubmit(models, (Naming.InvoiceStepDefinition)item.StepID, Naming.DataProcessStatus.Done);
                models.SubmitChanges();
        ...
        private int WriteG0501ToTurnkey(DataProcessQueue item)
        {
            ...
            if (allowance == null)
            {
                allowance = item.Doc.InvoiceAllowance;
            }
            ...
            try
            {
                ...
                if (allowance.InvoiceAllowanceSeller.Seller.OrganizationStatus.DownloadDispatch == true)
                {
                    item.Doc.PushStepQueueOnSubmit(models, Naming.InvoiceStepDefinition.回傳MIG, Naming.InvoiceProcessType.G0501);
                    models.SubmitChanges();
                }
        ...
        {
            int docID = item.DocID;
            var invoice = item.Doc.DerivedDocument?.ParentDocument.InvoiceItem;
            if (invoice == null)
            {
                invoice = item.Doc.InvoiceItem;
            }
            ...
            try
            {
                ...
                if (invoice.Seller!.OrganizationStatus?.DownloadDispatch == true)
                {
                    item.Doc.PushStepQueueOnSubmit(models, Naming.InvoiceStepDefinition.回傳MIG, Naming.InvoiceProcessType.F0501);
                    models.SubmitChanges();
                }
        ...
        //                if (bSigned)
        //                {
        //                    _table.Add(new A0101DispatchQueue
        //                    {
        //                        DocID = item.DocID,
        ...
        {
            item.Doc.PushLogOnSubmit(models, (Naming.InvoiceStepDefinition)item.StepID, Naming.DataProcessStatus.Done);
            item.Doc.PushStepQueueOnSubmit(models, targetStep, targetProcessType);
            models.SubmitChanges();
        ...
        {
            item.Doc.PushLogOnSubmit(models, (Naming.InvoiceStepDefinition)item.StepID, Naming.DataProcessStatus.Done, processType: (Naming.InvoiceProcessType)item.ProcessType);
            models.SubmitChanges();
        ...
        public static DataProcessQueue? GetReadyItem(GenericDbContext<ApplicationDbContext> models, CDS_Document docItem, Naming.InvoiceStepDefinition stepID, Naming.InvoiceProcessType processType)
        {
            lock (typeof(InvoiceHandler))
            {
                DateTime available = DateTime.Now.AddMinutes(-5);
