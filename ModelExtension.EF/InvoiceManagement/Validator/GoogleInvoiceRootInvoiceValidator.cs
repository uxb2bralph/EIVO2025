...
namespace ModelCore.InvoiceManagement.Validator
{
    ...
    {
        public GoogleInvoiceRootInvoiceValidator(GenericDbContext<ApplicationDbContext> mgr, Organization owner)
            : base(mgr, owner)
        {
        ...
        public InvoiceItem SaveRootInvoice(InvoiceRootInvoice invItem, bool forTerms, String invoiceClientID, int? channelID, out Exception exception)
        {
            ...
            //else
            //{
            //    C0401Handler.PushStepQueueOnSubmit(_mgr, newItem.Doc, Naming.InvoiceStepDefinition.已接收資料待通知);
            //}

            //yuki 儲存發票
            _models.GetTable<InvoiceItem>().Add(newItem);
            ...
            //C0401Handler.PushProcessRequestDocumentOnSubmit(_mgr, newItem.Doc, taskID);
            ...
            else
            {
                ...
                {
                    //newItem.Doc.PushStepQueueOnSubmit(_models, Naming.InvoiceStepDefinition.已接收資料待通知, Naming.InvoiceProcessType.F0401);
                }
                _models.SubmitChanges();
            }
