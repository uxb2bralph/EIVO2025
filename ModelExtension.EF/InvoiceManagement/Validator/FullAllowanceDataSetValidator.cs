...
namespace ModelCore.InvoiceManagement.Validator
{
    ...
    {
        public FullAllowanceDataSetValidator(GenericDbContext<ApplicationDbContext> mgr, Organization owner) : base(mgr, owner)
        {
        ...
        protected override Exception CheckAllowanceItem()
        {
            ...
            {
                allowanceItem.Amount = ((originalInvoice.InvoiceAmountType.TotalAmount ?? 0) / 1.05M).ToFix(originalInvoice.InvoiceAmountType.Currency?.Decimals ?? 0);
            ...
            if (productItems.Count() == 1)
            {
                ...
                allowanceItem.OriginalDescription = productItem.Product.Brief;
