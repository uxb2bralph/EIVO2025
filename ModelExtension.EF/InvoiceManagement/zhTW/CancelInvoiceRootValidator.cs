...
namespace ModelCore.InvoiceManagement.zhTW
{
    ...
    {
        //檢查基本必填項目(作廢發票)
        public static Exception? CheckMandatoryFields(this CancelInvoiceRootCancelInvoice invItem, GenericDbContext<ApplicationDbContext> mgr, OrganizationToken? owner, out InvoiceItem? invoice, out DateTime cancelDate, out bool duplicateProcess, Naming.InvoiceProcessType? processType = Naming.InvoiceProcessType.F0501)
        {
        ...
        //    if (invoice.InvoiceSeller.Company.ReceiptNo != invItem.SellerId)
        //    {
        //        return new Exception(String.Format("作廢之發票非原發票開立人,發票號碼:{0}", invItem.CancelInvoiceNumber));
        //    }
