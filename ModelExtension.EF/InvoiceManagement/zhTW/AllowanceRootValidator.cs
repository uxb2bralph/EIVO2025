...
namespace ModelCore.InvoiceManagement.zhTW
{
    ...
    {
        public static Exception CheckBusiness(this AllowanceRootAllowance item, GenericDbContext<ApplicationDbContext> mgr, OrganizationToken owner, out Organization seller)
        {
        ...
        public static Exception CheckBusiness_Proxy(this AllowanceRootAllowance item, GenericDbContext<ApplicationDbContext> mgr, OrganizationToken owner, out Organization seller)
        {
            seller = mgr.GetTable<Organization>().Where(o => o.ReceiptNo == item.SellerId).FirstOrDefault();
        ...
        //檢查基本必填項目(折讓單)
        public static Exception CheckMandatoryFields(this AllowanceRootAllowance item, GenericDbContext<ApplicationDbContext> mgr, out DateTime allowanceDate)
        {
        ...
        public static Exception CheckAllowanceItem(this AllowanceRootAllowance item, GenericDbContext<ApplicationDbContext> mgr, out List<InvoiceAllowanceItem> productItems,out InvoiceItem originalInvoiceItem)
        {
            ...
            foreach (var i in item.AllowanceItem)
            {
                ...
                var invProductItem = originalInvoice.Product.Join(mgr.GetTable<InvoiceProductItem>(), d => d.ProductID, p => p.ProductID, (d, p) => p)
