...
namespace ModelCore.InvoiceManagement.enUS
{
    public static partial class CancelInvoiceRootValidator
    {
        ...
        public static Exception CheckMandatoryFields(this CancelInvoiceRootCancelInvoice invItem, GenericDbContext<ApplicationDbContext> mgr, OrganizationToken owner, out InvoiceItem invoice, out DateTime cancelDate)
        {
            invoice = null;
            ...
            else
            {
                invoice = mgr.GetTable<InvoicePurchaseOrder>().Where(p => p.OrderNo == invItem.CancelDataNumber)
                            .Select(p => p.Invoice)
                            .Where(i => i.Seller.ReceiptNo == invItem.SellerId)
                            .FirstOrDefault();
            ...
            if (invoice.Seller.ReceiptNo != invItem.SellerId)
            {
        ...
        public static Exception? VoidInvoice(this DataRow item, GenericDbContext<ApplicationDbContext> models, Organization owner, ref InvoiceCancellation voidItem, ref DerivedDocument? p,ref Organization? expectedSeller)
        {
            ...
            if (models.GetTable<InvoiceAllowanceItem>()
                .Where(a => a.InvoiceNo == expectedNo)
