...
namespace ModelCore.InvoiceManagement
{
    public class InvoiceManagerForCBE : InvoiceManagerV3
    {
        ...
        public InvoiceManagerForCBE(GenericDbContext<ApplicationDbContext> mgr) : base(mgr) { }
        ...
        public override Dictionary<int, Exception> SaveUploadInvoice(InvoiceRoot item, OrganizationToken owner)
        {
            ...
            if (item != null && item.Invoice != null && item.Invoice.Length > 0)
            {
                ...
                InvoiceRootInvoiceValidatorForCBE validator = new InvoiceRootInvoiceValidatorForCBE(this, owner.Company);
        ...
        public override Dictionary<int, Exception> SaveUploadInvoiceAutoTrackNo(InvoiceRoot item, OrganizationToken owner)
        {
            ...
            if (item != null && item.Invoice != null && item.Invoice.Length > 0)
            {
                ...
                InvoiceRootInvoiceValidatorForCBE validator = new InvoiceRootInvoiceValidatorForCBE(workingMgr, owner.Company);
                ...
                for (int idx = 0; idx < item.Invoice.Length; idx++, count++)
                {
                    if (count == dbCheckCount)
                    {
                        ...
                        validator = new InvoiceRootInvoiceValidatorForCBE(workingMgr, owner.Company);
                        validator.StartAutoTrackNo(ApplyInvoiceDate);
                    }
                    ...
                    try
                    {
                        ...
                        if (ex != null)
                        {
                            ...
                            {
                                newItem = ((DuplicateDataNumberException)ex).CurrentPO.Invoice;
                                if (newItem == null)
                                {
                                    result.Add(idx, ex);
                                    continue;
