...
namespace ModelCore.InvoiceManagement
{
    public class InvoiceManager: EIVOEntityManager<InvoiceItem>
    {
        ...
        public InvoiceManager(GenericDbContext<ApplicationDbContext> mgr) : base(mgr) { }
        ...
        public virtual Dictionary<int, Exception> ReviseInvoice(InvoiceRoot item, OrganizationToken? owner)
        {
            Dictionary<int, Exception> result = new Dictionary<int, Exception>();
            ...
            {
                List<InvoiceItem> eventItems = new List<InvoiceItem>();
                InvoiceRootInvoiceValidatorForRevision validator = new InvoiceRootInvoiceValidatorForRevision(this, owner?.Company)
                {
                ...
                for (int idx = 0; idx < item.Invoice.Length; idx++)
                {
                    try
                    {
                        ...
                        InvoiceItem newItem = validator.InvoiceItem;
                        InvoiceItem original = validator.Original!;
                        ...
                        this.ProcessVoidInvoiceRequest(Naming.VoidActionMode.註銷重開, null, original);
                ...
                for (int idx = 0; idx < item.CancelInvoice.Length; idx++)
                {
                    ...
                    try
                    {
                        ...
                        if (!duplicateProcess)
                        {
                            ...
                            this.SubmitChanges();
                            eventItems.Add(cancelItem.Invoice);
                        }
                        else
                        {
        ...
        {
            return this.GetTable<WelfareReplication>().Where(w => w.Welfare.Seller.ReceiptNo == receiptNo);
        }

        public IEnumerable<InvoiceWelfareAgency> GetWelfareAgenciesForSeller(String receiptNo)
        {
            return this.GetTable<InvoiceWelfareAgency>().Where(w => w.Seller.ReceiptNo == receiptNo);
        }
        ...
        public virtual Dictionary<int, Exception> SaveUploadAllowance(AllowanceRoot item, OrganizationToken? owner)
        {
            ...
            if (item != null && item.Allowance != null && item.Allowance.Length > 0)
            {
                ...
                AllowanceRootAllowanceValidator validator = new AllowanceRootAllowanceValidator(this, owner?.Company)
                {
                    ProcessType = item.ProcessType != null ? Enum.Parse<Naming.InvoiceProcessType>(item.ProcessType) : this.ProcessType,
                };
        ...
        //public CDS_Document CreateInvoiceFromShipment(SCMDataEntity.CDS_Document item,int sellerID)
        //{
        //    Company seller = this.GetTable<Company>().Where(o => o.CompanyID == sellerID).FirstOrDefault();
        //    if (seller == null)
        //    {
        ...
        //            //InvoiceByHousehold = carrier != null ? new InvoiceByHousehold { Carrier = carrier } : null,
        ...
        //        {
        //            Product = new Product { Brief = o.PRODUCTS_DATA.PRODUCTS_NAME },
        ...
        //        newItem.Product.AddRange(productItems.Select(p => new InvoiceDetail
        //        {
        //            Product = p.Product
        //        }));
        ...
        public InvoiceCancellation CreateInvoiceCancellationFromReturnedGoods(int invoiceID,String remark)
        {
            ...
            this.GetTable<DerivedDocument>().Add(doc);
            ...
            if (item != null && item.Invoice != null && item.Invoice.Length > 0)
            {
                ...
                for (int idx = 0; idx < item.Invoice.Length; idx++)
                {
                    try
                    {
                        ...
                        //        continue;
                        //    }
                        //    donatory = this.GetTable<Company>().Where(o => o.ReceiptNo == invItem.NPOBAN).FirstOrDefault();
                        //    if (donatory == null || !this.GetTable<Welfare>().Any(w => w.AgencyID == donatory.CompanyID && w.SellerID == seller.CompanyID))
                        //    {
            ...
            if (root != null && root.CancelAllowance != null && root.CancelAllowance.Length > 0)
            {
                ...
                for (int idx = 0; idx < root.CancelAllowance.Length; idx++)
                {
                    ...
                    try
                    {
                        ...
                        if ((ex = item.VoidAllowance(this, owner?.Company, ref voidItem, ref derivedDoc, out bool duplicateProcess, processType)) != null)
                        {
                            result.Add(idx, ex);
                            continue;
                        }
                        ...
                        eventItems.Add(voidItem!.Allowance);
                    }
                    catch (Exception ex)
                    {
                        Logger.Error(ex);
