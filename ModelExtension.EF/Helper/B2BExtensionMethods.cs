    ...
    public static class B2BExtensionMethods
    {
        ...
        private static AllowanceRootAllowance[] buildAllowanceRootAllowance(IEnumerable<InvoiceAllowance> items)
        {
            ...
            return items.Select<InvoiceAllowance, AllowanceRootAllowance>((Func<InvoiceAllowance, AllowanceRootAllowance>)(a =>
            {
                ...
                allowanceRootAllowance.AllowanceItem = a.InvoiceAllowanceDetails.Select<InvoiceAllowanceItem, AllowanceRootAllowanceAllowanceItem>((Func<InvoiceAllowanceItem, AllowanceRootAllowanceAllowanceItem>)(d =>
                {
                    AllowanceRootAllowanceAllowanceItem allowanceAllowanceItem = new AllowanceRootAllowanceAllowanceItem();
                    allowanceAllowanceItem.AllowanceSequenceNumber = d.No ?? itemIdx++;
                    allowanceAllowanceItem.Amount = d.Amount.HasValue ? d.Amount.Value : Decimal.Zero;
                    //allowanceAllowanceItem.Amount2 = d.Amount2 ?? 0;
                    allowanceAllowanceItem.OriginalInvoiceDate = a.Invoice == null ? string.Format("{0:yyyy/MM/dd}", (object)d.InvoiceDate) : string.Format("{0:yyyy/MM/dd}", (object)a.Invoice.InvoiceDate);
                    allowanceAllowanceItem.OriginalDescription = d.OriginalDescription;
                    allowanceAllowanceItem.OriginalInvoiceNumber = a.Invoice == null ? d.InvoiceNo : string.Format("{0}{1}", (object)a.Invoice.TrackCode, (object)a.Invoice.No);

                    allowanceAllowanceItem.Quantity = d.Piece ?? 0;

                    //allowanceAllowanceItem.Quantity2 = d.Piece2 ?? 0;
                    allowanceAllowanceItem.Tax = d.Tax ?? 0;
                    allowanceAllowanceItem.TaxType = d.TaxType.Value;
                    allowanceAllowanceItem.Unit = d.PieceUnit;
                    allowanceAllowanceItem.UnitPrice = d.UnitCost ?? 0;
                    //allowanceAllowanceItem.Unit2 = d.PieceUnit2;

                    //allowanceAllowanceItem.UnitPrice2 = d.UnitCost2 ?? 0;
                    allowanceAllowanceItem.Remark = d.Remark;
                    allowanceAllowanceItem.OriginalSequenceNumber = d.OriginalSequenceNo ?? 1;
                    return allowanceAllowanceItem;
                })).ToArray<AllowanceRootAllowanceAllowanceItem>();
                allowanceRootAllowance.AllowanceDate = string.Format("{0:yyyy/MM/dd}", a.AllowanceDate);
                Decimal? nullable1;
                Decimal zero4;
        ...
        public static X509Certificate2 PrepareSignerCertificate(this Organization item)
        {
            ...
            var signerToken = item.OrganizationStatus.Token;
