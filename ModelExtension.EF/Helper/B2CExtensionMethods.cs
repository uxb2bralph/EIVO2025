...
namespace ModelCore.Helper
{
    public static class B2CExtensionMethods
    {
        ...
        //                PersonInCharge = item.Company.UndertakerName.GetEfficientStringMaxSize(0, 30).InsteadOfNullOrEmpty(""),
        ...
        //            SalesAmount = item.InvoiceBuyer.IsB2C() && item.InvoiceAmountType.TaxType == (byte)Naming.TaxTypeDefinition.應稅 ? item.InvoiceAmountType.TotalAmount.ToFix(item.InvoiceAmountType.Currency?.Decimals ?? 0) : item.InvoiceAmountType.SalesAmount.ToFix(item.InvoiceAmountType.Currency?.Decimals ?? 0),
        //            FreeTaxSalesAmount = item.InvoiceAmountType.FreeTaxSalesAmount.ToFix(item.InvoiceAmountType.Currency?.Decimals ?? 0),
        ...
        //                UnitPrice = productItem.UnitCost ?? 0m, //productItem.UnitCost.HasValue ? productItem.UnitCost.Value.ToFix(item.InvoiceAmountType.Currency?.Decimals ?? 0) : 0,
        //            });
        //        }
        //    }
        ...
        //        SellerId = item.Company.ReceiptNo,
        ...
        //                    {
        //                        TaxAmount = 0,
        //                        TotalAmount = ((item.TotalAmount ?? 0) + (item.TaxAmount ?? 0)).ToFix(item.Currency?.Decimals ?? 0),
        //                    }
        //                    : */
        //                    new Schema.TurnKey.D0401.Amount
        //                    {
        //                        TaxAmount = item.TaxAmount.HasValue
        //                                    ? item.TaxAmount.Value.ToFix(item.Currency?.Decimals ?? 0)
        //                                    : 0,
        //                        TotalAmount = item.TotalAmount.HasValue ? item.TotalAmount.Value.ToFix(item.Currency?.Decimals ?? 0) : 0,
        //                    },
        //    };
        ...
        //    {
        //        AllowanceSequenceNumber = (seqNo++).ToString(), //d.No.ToString(),
        //        Amount = d.Amount.HasValue ? d.Amount.Value.ToFix(item.Currency?.Decimals ?? 0) : 0m,
        //        OriginalDescription = d.OriginalDescription,
        //        OriginalInvoiceDate = String.Format("{0:yyyyMMdd}", d.InvoiceDate),
        //        OriginalInvoiceNumber = d.InvoiceNo,
        //        OriginalSequenceNumber = d.OriginalSequenceNo?.ToString() ?? "1",
        //        Quantity = (int)(d.Piece ?? 0M),
        //        Tax = (int)(d.Tax ?? 0),
        //        TaxType = (Schema.TurnKey.D0401.TaxTypeEnum?)d.TaxType ?? Schema.TurnKey.D0401.TaxTypeEnum.Item1,
        //        Unit = d.PieceUnit,
        ...
        public static ModelCore.Schema.TurnKey.E0402.BranchTrackBlank CreateE0402(this InvoiceTrackCodeAssignment item)
        {
            var result = new ModelCore.Schema.TurnKey.E0402.BranchTrackBlank
            {
                ...
                {
                    HeadBan = item.Seller.Headquarter?.ReceiptNo ?? item.Seller.ReceiptNo,
                    BranchBan = item.Seller.ReceiptNo,
                    InvoiceType = item.Track .InvoiceType == (byte)Schema.TurnKey.E0402.InvoiceTypeEnum.Item08 ? Schema.TurnKey.E0402.InvoiceTypeEnum.Item08 : Schema.TurnKey.E0402.InvoiceTypeEnum.Item07,
                    YearMonth = String.Format("{0:000}{1:00}", item.Track.Year - 1911, item.Track.PeriodNo * 2),
                    InvoiceTrack = item.Track.TrackCode
                },
        ...
        public static ModelCore.Schema.TurnKey.E0402.BranchTrackBlank BuildE0402(this InvoiceTrackCodeAssignment item, IQueryable<UnassignedInvoiceNoSummary> Summary)
        {
            var result = new ModelCore.Schema.TurnKey.E0402.BranchTrackBlank
            {
                Main = new Schema.TurnKey.E0402.Main
                {
                    HeadBan = item.Seller.ReceiptNo,
                    BranchBan = item.Seller.ReceiptNo,
                    InvoiceType = (Schema.TurnKey.E0402.InvoiceTypeEnum)(item.Seller.InvoiceItems.Where(i => i.InvoiceDate >= Convert.ToDateTime(String.Format("{0}/{1}/1", item.Track.Year, item.Track.PeriodNo * 2 - 1))).First().InvoiceType!),
                    YearMonth = String.Format("{0:000}{1:00}", item.Track.Year - 1911, item.Track.PeriodNo * 2),
                    InvoiceTrack = item.Track.TrackCode
                },
        ...
        public static XmlDocument CreateA0101(this InvoiceItem item)
        {
            ...
            a0101!.Amount!.SalesAmount = a0101.Amount.SalesAmount > 0
                    ? a0101.Amount.SalesAmount.ToFix(item.InvoiceAmountType?.Currency?.Decimals ?? 0)
                    : a0101.Amount.TotalAmount.ToFix(item.InvoiceAmountType?.Currency?.Decimals ?? 0);
            a0101.Amount.TaxAmount = a0101.Amount.TaxAmount.ToFix(item.InvoiceAmountType?.Currency?.Decimals ?? 0);
            a0101.Amount.TotalAmount = a0101.Amount.TotalAmount.ToFix(item.InvoiceAmountType?.Currency?.Decimals ?? 0);
            a0101.Amount.DiscountAmount = a0101.Amount.DiscountAmount.ToFix(item.InvoiceAmountType?.Currency?.Decimals ?? 0);
            ...
            //a0101!.Amount!.FreeTaxSalesAmount = a0101.Amount.FreeTaxSalesAmount.ToFix(item.InvoiceAmountType.Currency?.Decimals ?? 0);
            //a0101!.Amount!.ZeroTaxSalesAmount = a0101.Amount.ZeroTaxSalesAmount.ToFix(item.InvoiceAmountType.Currency?.Decimals ?? 0);
        ...
        public static ModelCore.Schema.TurnKey.Allowance.Allowance CreateAllowanceMIG(this InvoiceAllowance item, GenericDbContext<ApplicationDbContext>? models = null, bool withExtension = false)
        {
            ...
            var result = new Schema.TurnKey.Allowance.Allowance
            {
                ...
                Amount = /*isCBM
                            ...
                            {
                                TaxAmount = item.TaxAmount.HasValue
                                            ? item.TaxAmount.Value.ToFix(0 /*item.Currency?.Decimals ?? 0*/)
                                            : 0,
                                TotalAmount = item.TotalAmount.HasValue ? item.TotalAmount.Value.ToFix(item.Currency?.Decimals ?? 0) : 0,
                            },
            };
            ...
            result.Details = item.InvoiceAllowanceDetails.Select(d => new Schema.TurnKey.Allowance.DetailsProductItem
            {
                ...
                Quantity = (int)(d.Piece ?? 0M),
                Tax = (int)(d.Tax ?? 0),
                TaxType = (TaxTypeEnum?)d.TaxType ?? TaxTypeEnum.Item1,
                Unit = d.PieceUnit,
                UnitPrice = d.UnitCost ?? 0,
            }).ToArray();
        ...
        public static Invoice CreateInvoiceMIG(this InvoiceItem item, bool withExtension = false)
        {
            var result = new Invoice
            {
                Main = new Main
                {
                    ...
                    Seller = new RoleDescription
                    {
                        ...
                        Identifier = item.InvoiceSeller?.ReceiptNo,
                        Name = item.InvoiceSeller?.CustomerName.GetEfficientStringMaxSize(0, 60).InsteadOfNullOrEmpty(""),
                        PersonInCharge = item.Seller.UndertakerName.GetEfficientStringMaxSize(0, 30).InsteadOfNullOrEmpty(""),
                        //RoleRemark = item.InvoiceSeller.RoleRemark.GetEfficientStringMaxSize(0, 40).InsteadOfNullOrEmpty(""),
                        TelephoneNumber = item.InvoiceSeller.Phone.GetEfficientStringMaxSize(0, 26).InsteadOfNullOrEmpty("")
                    },
                },
                ...
                Amount = new Amount
                {
                    ...
                    SalesAmount = item.InvoiceBuyer.IsB2C() && item.InvoiceAmountType.TaxType == (byte)Naming.TaxTypeDefinition.應稅
                                    ? item.InvoiceAmountType.TotalAmount.ToFix(item.InvoiceAmountType.Currency?.Decimals ?? 0)
                                    : item.InvoiceAmountType.SalesAmount.ToFix(item.InvoiceAmountType.Currency?.Decimals ?? 0),
                    FreeTaxSalesAmount = item.InvoiceAmountType.FreeTaxSalesAmount.ToFix(item.InvoiceAmountType.Currency?.Decimals ?? 0),
                    ZeroTaxSalesAmount = item.InvoiceAmountType.ZeroTaxSalesAmount.ToFix(item.InvoiceAmountType.Currency?.Decimals ?? 0),
                    TaxAmount = item.InvoiceBuyer.IsB2C() ? 0 : item.InvoiceAmountType.TaxAmount.ToFix(0 /*item.InvoiceAmountType.Currency?.Decimals ?? 0*/),
                    TaxRate = item.Seller?.UseDefaultTaxRate() == true && item.InvoiceAmountType.TaxType == (byte)Naming.TaxTypeDefinition.應稅
            ...
            {
                result.Amount.CurrencySpecified = true;
                result.Amount.Currency = (CurrencyCodeEnum)Enum.Parse(typeof(Schema.TurnKey.Invoice.CurrencyCodeEnum), item.InvoiceAmountType.Currency.AbbrevName);
            }
            if (item.InvoiceAmountType.TaxType == (byte)Naming.TaxTypeDefinition.零稅率)
            {
                ...
                else if (item.Seller.OrganizationCustomSetting?.Settings?.ZeroTaxRateReason.HasValue == true)
                {
                    result.Main.ZeroTaxRateReason = item.Seller.OrganizationCustomSetting.Settings.ZeroTaxRateReason.Value;
                    result.Main.ZeroTaxRateReasonSpecified = true;
                }
            }
        ...
        public static CancelInvoice? CreateCancelInvoiceMIG(this InvoiceItem item, bool withExtension = false)
        {
            ...
            var result = new ModelCore.Schema.TurnKey.Invoice.CancelInvoice
            {
                ...
                InvoiceDate = String.Format("{0:yyyyMMdd}", item.InvoiceDate),
                BuyerId = item.InvoiceBuyer.ReceiptNo,
                SellerId = item.Seller.ReceiptNo,
        ...
        private static DetailsProductItem[] buildF0401Details(InvoiceItem item)
        {
            ...
            foreach (var detailItem in item.Product)
            {
                detailItem.InvoiceProductItem.ToList();
                foreach (var productItem in detailItem.InvoiceProductItem)
                {
                    items.Add(new DetailsProductItem
                    {
                        Amount = productItem.CostAmount ?? 0m,   //productItem.CostAmount.HasValue ? productItem.CostAmount.Value.ToFix(item.InvoiceAmountType.Currency?.Decimals ?? 0) : 0m,
                        Description = detailItem.Brief,
                        Quantity = productItem.Piece ?? 0m,  //productItem.Piece.HasValue ? productItem.Piece.Value.ToFix(0) : 0,
                        RelateNumber = productItem.RelateNumber,
                        Remark = productItem.Remark.GetEfficientStringMaxSize(0, 40),
                        SequenceNumber = String.Format("{0:00}", productItem.No),
        ...
        public static XmlDocument CreateG0401(this InvoiceAllowance item, GenericDbContext<ApplicationDbContext>? models = null, bool withExtension = false)
        {
        ...
        public static XmlDocument? CreateB0101(this InvoiceAllowance item, GenericDbContext<ApplicationDbContext>? models = null)
        {
