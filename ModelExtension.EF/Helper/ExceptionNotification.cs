    ...
    public static class ExceptionNotification
    {
                ...
                if (items.Count() > 0)
                {
                    ...
                    foreach (var item in items.GroupBy(i => i.CompanyID))
                    {
                        if (item.Key.HasValue)
                        {
                            ...
                            {
                                CompanyID = item.Key,
                                EMail = item.First().Company.ContactEmail,
                                MaxLogID = maxLogID
                            });
                        }
                    }
                    ...
                    var target = items
                        ...
                        .Where(i => i.ExceptionReplication != null)
                        .Select(i => i.ExceptionReplication!);
                    mgr.GetTable<ExceptionReplication>().RemoveRange(target);
                    mgr.SubmitChanges();
                }
            }
        }
        ...
        private static void notifyExceptionWhenUploadInvoice(ExceptionInfo info)
        {
            ...
            using (InvoiceManager mgr = new InvoiceManager())
            {
                ...
                table.AddRange(info.ExceptionItems.Select(e => new ExceptionLog
                {
                    CompanyID = companyID,
        ...
        private static void notifyExceptionWhenUploadCsvInvoice(ExceptionInfo info)
        {
            ...
            using (InvoiceManager mgr = new InvoiceManager())
            {
                ...
                table.AddRange(info.InvoiceError.Select(e => new ExceptionLog
                {
                    CompanyID = companyID,
                    LogTime = DateTime.Now,
                    TypeID = (int)Naming.DocumentTypeDefinition.E_Invoice,
        ...
        private static void notifyExceptionWhenUploadCsvInvoiceCancellation(ExceptionInfo info)
        {
            ...
            using (InvoiceManager mgr = new InvoiceManager())
            {
                ...
                table.AddRange(info.InvoiceCancellationError.Select(e => new ExceptionLog
                {
                    CompanyID = companyID,
                    LogTime = DateTime.Now,
        ...
        private static void notifyExceptionWhenUploadCsvAllowance(ExceptionInfo info)
        {
            ...
            using (InvoiceManager mgr = new InvoiceManager())
            {
                ...
                table.AddRange(info.AllowanceError.Select(e => new ExceptionLog
                {
                    CompanyID = companyID,
                    LogTime = DateTime.Now,
        ...
        private static void notifyExceptionWhenUploadCsvAllowanceCancellation(ExceptionInfo info)
        {
            ...
            using (InvoiceManager mgr = new InvoiceManager())
            {
                ...
                table.AddRange(info.AllowanceCancellationError.Select(e => new ExceptionLog
                {
                    CompanyID = companyID,
                    LogTime = DateTime.Now,
        ...
        private static void notifyExceptionWhenUploadCounterpartBusiness(ExceptionInfo info)
        {
            ...
            using (InvoiceManager mgr = new InvoiceManager())
            {
                ...
                table.AddRange(info.CounterpartBusinessError.Select(e => new ExceptionLog
                {
                    CompanyID = companyID,
                    LogTime = DateTime.Now,
                    TypeID = (int)Naming.DocumentTypeDefinition.E_InvoiceCancellation,
