...
namespace Business.Helper.BillingSettlement
{
    ...
    {
        public static Settlement AssertMonthlyBillingSettlement(this DateTime settlementDate, GenericDbContext<ApplicationDbContext> models)
        {
            lock (typeof(ExtensionMethods))
            {
                ...
                if (settlement == null)
                {
                    ...
                    models.GetTable<Settlement>().Add(settlement);
        ...
        public static void DoMonthlyBillingSettlement(this Settlement settlement, GenericDbContext<ApplicationDbContext> models)
        {
            lock (typeof(ExtensionMethods))
            {
                ...
                foreach(var item in orgItems) 
                {
                    ...
                    if (billing == null)
                    {
                        ...
                        models.GetTable<MonthlyBilling>().Add(billing);
                    }

                    billing.TotalIssueCount = models.GetInvoice(item.CompanyID, settlement.StartDate, settlement.EndExclusiveDate).Count();
                    if (item.Type.Any(c => c.TypeID == (int)Naming.DocumentTypeDefinition.E_InvoiceCancellation))
                    {
                    ...
                    if (item.Type.Any(c => c.TypeID == (int)Naming.DocumentTypeDefinition.E_Allowance))
                    {
                        billing.TotalIssueCount += models.GetAllowance(item.CompanyID, settlement.StartDate, settlement.EndExclusiveDate).Count();
                    }
                    if (item.Type.Any(c => c.TypeID == (int)Naming.DocumentTypeDefinition.E_AllowanceCancellation))
                    {
        ...
        public static void DoSubmitBill(this GenericDbContext<ApplicationDbContext> models)
        {
            lock (typeof(ExtensionMethods))
            {
