...
namespace ModelCore.InvoiceManagement.InvoiceProcess
{
    public class A0401Handler
    {
        static A0401Handler()
        {
            ModelExtension.Properties.AppSettings.Default.A0401Outbound.CheckStoredPath();
        }
        ...
        public void WriteToTurnkey()
        {
            ...
            while (buffer.Count > 0)
            {
                foreach (var item in buffer)
                {
                    ...
                    try
                    {
                        var fileName = Path.Combine(ModelExtension.Properties.AppSettings.Default.A0401Outbound, $"A0401-{invoiceItem.InvoiceID}-{invoiceItem.TrackCode}{invoiceItem.No}.xml");
