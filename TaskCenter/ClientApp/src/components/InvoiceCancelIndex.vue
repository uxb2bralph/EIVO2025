<!--
  InvoiceCancelIndex.vue — 線上作廢發票。
  遷移自 WebHome InvoiceProcessController.InquireToCancel（選單「線上作廢發票」/InvoiceProcess/InquireToCancel）。
  查詢與「資料查詢／列印／匯出」相同（重用 useInvoiceProcessIndex），差異在於：
    - 結果列可勾選，並以「執行作廢」批次作廢選取之發票（對應舊版 CancelInvoice CommitAction）。
    - 預設僅查詢未作廢發票；已作廢之發票不可勾選。
  查詢資料範圍與作廢對象皆由後端依登入者角色過濾。
-->
<script setup lang="ts">
import { ref, watch } from 'vue'
import { useInvoiceProcessIndex } from '../composables/useInvoiceProcessIndex'
import { useTableSelection } from '../composables/useTableSelection'
import { cancelInvoices, type InvoiceItemDatatable } from '@/services/invoice-process-api'
...
// ── 列勾選 ──────────────────────────────────────────────────
// 以 keyId（加密 InvoiceID）為選取鍵；已作廢或無 keyId 的列不可選。
function selectableKey(row: InvoiceItemDatatable): string | null {
  return row.isCancelled ? null : row.keyId
}
...
async function onCancel() {
  ...
  if (!keys.length) {
    window.alert('請選擇作廢資料!!')
    return
  }
  if (!window.confirm(`確定要作廢選取的 ${keys.length} 筆發票嗎？此動作無法復原。`)) return

  cancelling.value = true
  resultMsg.value = ''
  resultError.value = ''
  try {
    const res = await cancelInvoices(keys)
    if (res.success && res.data) {
      const d = res.data
      const lines: string[] = []
      if (d.cancelledCount > 0) {
        lines.push(`已作廢 ${d.cancelledCount} 筆：${d.cancelledNos.join('、')}`)
      }
      if (d.skippedNos.length) {
        lines.push(`未處理 ${d.skippedNos.length} 筆（已作廢／無權存取）：${d.skippedNos.join('、')}`)
      }
      if (!lines.length) lines.push('沒有可作廢的發票。')
      resultMsg.value = lines.join('\n')
      clear()
      onSearch() // 重新查詢以反映最新狀態
    } else {
      resultError.value = res.errors?.length ? res.errors.join('\n') : res.message || '作廢失敗'
    }
  } catch (e) {
    resultError.value = e instanceof Error ? e.message : '作廢失敗'
  } finally {
...
<template>
  <div class="invoice-page">
    ...
    <div class="card result-card">
      ...
      <div v-if="resultMsg" class="alert-info">{{ resultMsg }}</div>
      ...
      <div v-else>
        <div class="batch-bar">
          ...
          <button class="btn danger sm" :disabled="!selected.length || cancelling" @click="onCancel">
            {{ cancelling ? '作廢中…' : '執行作廢' }}
          </button>
        </div>
            ...
            <tr v-for="row in items" :key="row.invoiceId" :class="{ 'row-void': row.isCancelled }">
              <td class="check-col">
                <input
                  ...
                  :disabled="row.isCancelled || !row.keyId"
                  :title="row.isCancelled ? '已作廢' : ''"
...
.btn.danger { background: #d9534f; border: 1px solid #d9534f; color: #fff; }
...
.alert-info { color: var(--color-text); background: rgba(59, 199, 255, 0.1); border: 1px solid var(--color-accent); border-radius: 0.5rem; padding: 0.75rem 1rem; margin-bottom: 1rem; white-space: pre-line; }
