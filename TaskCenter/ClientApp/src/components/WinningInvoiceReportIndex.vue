<!--
  WinningInvoiceReportIndex.vue — 中獎統計表。
  ...
  查詢表單沿用舊版 Module/InquireWinning.cshtml（開立人 / 發票日期 / 代理業者）；
  結果只計中獎發票，依開立人彙總「中獎張數 / 捐贈張數」，並可匯出 Excel。
  發票日期起迄為必填；資料範圍由後端依登入者角色過濾（本頁限系統管理使用）。
-->
<script setup lang="ts">
import { useWinningInvoiceReportIndex } from '../composables/useWinningInvoiceReportIndex'
    ...
    <div class="card result-card">
      ...
      <div v-else class="table-wrap">
        <table class="result-table">
          <thead>
            <tr>
              <th class="sortable" @click="toggleSort('SellerReceiptNo')">賣方統一編號{{ sortArrow('SellerReceiptNo') }}</th>
              <th class="sortable" @click="toggleSort('SellerName')">賣方名稱{{ sortArrow('SellerName') }}</th>
              <th>賣方地址</th>
              <th class="num sortable" @click="toggleSort('WinningCount')">中獎張數{{ sortArrow('WinningCount') }}</th>
              <th class="num sortable" @click="toggleSort('DonationCount')">捐贈張數{{ sortArrow('DonationCount') }}</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="row in items" :key="row.sellerKey ?? row.sellerReceiptNo ?? ''">
              <td>{{ row.sellerReceiptNo }}</td>
              <td>{{ row.sellerName }}</td>
              <td class="addr">{{ row.addr }}</td>
              <td class="num">{{ formatAmount(row.winningCount) }}</td>
              <td class="num">{{ formatAmount(row.donationCount) }}</td>
            </tr>
...
.result-table td.addr { white-space: normal; min-width: 16rem; }
