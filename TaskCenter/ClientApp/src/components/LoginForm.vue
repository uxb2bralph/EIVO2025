...
// 登入後欲返回的頁面（由 router 導向時帶上的 query）
...
// If already logged in, redirect to the target (returnUrl) or main page
onMounted(() => {
  if (auth.isLoggedIn.value) {
    router.replace(returnUrl.value || '/MainPage')
  }
})
...
const onSubmit = async () => {
  ...
  try {
    ...
    if (data?.success) {
      ...
      // 優先返回登入前頁面（returnUrl），其次採用後端建議的 redirectUrl
      const target = returnUrl.value || data.redirectUrl || '/MainPage'
      router.push(target)
    } else {
      state.value = 'error'
