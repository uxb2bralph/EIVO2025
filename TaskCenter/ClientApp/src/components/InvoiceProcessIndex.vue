<!--
  InvoiceProcessIndex.vue — 發票資料查詢／列印／匯出。
  遷移自 WebHome InvoiceProcessController.Index/Inquire（選單「資料查詢／列印／匯出」）。
  多條件查詢 + 分頁 + 排序 + 幣別統計 + Excel／買受人／ERP 匯出 + 發票明細預覽。
  查詢資料範圍由後端依登入者角色過濾（系統管理看全部、開立人看自家、代理看旗下、買受人看進項）。
  ...
  <div class="invoice-page">
    ...
    <div class="card query-card">
      <div class="query-grid">
        ...
        <div class="form-group">
          <label>&nbsp;</label>
          <label class="check-inline"><input v-model="filters.hasAddr" type="checkbox" /> 篩選有買受人地址</label>
        </div>
        <div class="form-group">
          <label>&nbsp;</label>
          <label class="check-inline"><input v-model="filters.currencySummary" type="checkbox" /> 顯示幣別統計</label>
        </div>
      </div>

      <div class="query-actions">
        <button class="btn ghost" :disabled="exporting" @click="doExportXlsx">
          {{ exporting ? '匯出中…' : 'Excel下載' }}
        </button>
        <button v-if="isAdmin" class="btn ghost" :disabled="exporting" @click="doExportBuyer">下載買受人資料</button>
        <button v-if="isAdmin" class="btn ghost" :disabled="exporting" @click="doExportErp">ERP下載</button>
    ...
    <div class="card result-card">
        ...
        <table class="result-table">
          <thead>
            <tr>
              ...
              <th>備註</th>
              <th>列印註記</th>
              <th class="sortable" @click="toggleSort('IsWinning')">是否中獎{{ sortArrow('IsWinning') }}</th>
              <th>載具號碼</th>
              <th>捐贈愛心碼</th>
              <th class="sortable" @click="toggleSort('CustomerID')">客戶 ID{{ sortArrow('CustomerID') }}</th>
              <th>email</th>
              <th>開立通知</th>
              <th v-if="isAdmin">地址</th>
              <th v-if="isAdmin">連絡人</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="row in items" :key="row.invoiceId">
              ...
              <td>{{ row.email }}</td>
              <td>{{ row.issuingNoticeDate ? formatDate(row.issuingNoticeDate) : '未通知' }}</td>
              <td v-if="isAdmin">{{ row.buyerAddress }}</td>
      ...
      <!-- 幣別統計 -->
      <div v-if="filters.currencySummary && summary.length" class="summary-wrap">
        <h4>幣別統計</h4>
        <table class="result-table">
          <thead>
            <tr><th>幣別</th><th class="num">筆數</th><th class="num">未稅金額</th><th class="num">稅額</th><th class="num">含稅金額</th></tr>
          </thead>
          <tbody>
            <tr v-for="row in summary" :key="row.currency ?? ''">
              <td>{{ row.currency || '(未指定)' }}</td>
              <td class="num">{{ formatAmount(row.count) }}</td>
              <td class="num">{{ formatAmount(row.salesAmount) }}</td>
              <td class="num">{{ formatAmount(row.taxAmount) }}</td>
              <td class="num">{{ formatAmount(row.totalAmount) }}</td>
            </tr>
...
.summary-wrap { margin-top: 1.5rem; }
.summary-wrap h4 { margin: 0 0 0.6rem; color: var(--color-text); font-size: 1rem; }
