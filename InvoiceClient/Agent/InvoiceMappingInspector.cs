    ...
    public class InvoiceMappingInspector : ServerInspector
    {
        ...
        public IEnumerable<XmlNode> ReceiveInvoices()
        {
            ...
            try
            {
                ...
                if (doc != null)
                {
                    String storedPath = Settings.Default.DownloadDataInAbsolutePath ? Settings.Default.DownloadInvoiceMapping : Path.Combine(Settings.Default.InvoiceTxnPath[0], Settings.Default.DownloadInvoiceMapping);
        ...
        public override void ExecutiveService(List<string> pathInfo)
        {
            ...
            if (items != null && items.Count() > 0)
                pathInfo.Add(Path.Combine(Settings.Default.InvoiceTxnPath[0], Settings.Default.DownloadInvoiceMapping));
