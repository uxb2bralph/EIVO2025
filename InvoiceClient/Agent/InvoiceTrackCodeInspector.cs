...
namespace InvoiceClient.Agent
{
    ...
    public class InvoiceTrackCodeInspector : ServerInspector
    {
        ...
        public bool ReceiveYearTrackCode()
        {
            ...
            try
            {
                String storedPath = Settings.Default.DownloadDataInAbsolutePath ? Settings.Default.TrackCodeFolder : Path.Combine(Settings.Default.InvoiceTxnPath[0], Settings.Default.TrackCodeFolder);
        ...
        public override void ExecutiveService(List<string> pathInfo)
        {
            if (ReceiveYearTrackCode())
                pathInfo.Add(Settings.Default.DownloadDataInAbsolutePath ? Settings.Default.TrackCodeFolder : Path.Combine(Settings.Default.InvoiceTxnPath[0], Settings.Default.TrackCodeFolder));
